class AdminMailer < ActionMailer::Base
  ...
  def portal_for(resource)
    return t('titles.admin', brand: t('titles.brand')) if resource.is_a?(Admin::User)
    return t('titles.developer', brand: t('titles.brand')) if resource.is_a?(User)
  end

  def sign_in_url_for(resource)
    return new_admin_session_url(subdomain: ENV['ADMIN_SUBDOMAIN']) if resource.is_a?(Admin::User)
    return new_user_session_url(subdomain: ENV['DEV_SUBDOMAIN']) if resource.is_a?(User)
  end

  def sign_up_url_for(resource)
    return new_admin_registration_url(subdomain: ENV['ADMIN_SUBDOMAIN']) if resource.is_a?(Admin::User)
    return new_user_registration_url(subdomain: ENV['DEV_SUBDOMAIN']) if resource.is_a?(User)
  end

  def password_url_for(resource)
    return new_admin_password_url(subdomain: ENV['ADMIN_SUBDOMAIN']) if resource.is_a?(Admin::User)
